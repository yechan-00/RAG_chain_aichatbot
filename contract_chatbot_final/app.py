...
if st.session_state.analysis_done and st.session_state.analysis_result:
    ...
    # PDF 생성 함수 (Arial + 라틴1 인코딩)
        # PDF 생성 함수 (한글은 무시하고 Latin-1로 인코딩)
    def generate_pdf(summary: str, risks: str, score: int) -> bytes:
        # 1) 한글 등 Latin-1에 없는 문자는 모두 삭제
        summary_clean = summary.encode('latin-1', 'ignore').decode('latin-1')
        risks_clean   = risks.encode('latin-1',   'ignore').decode('latin-1')
        ...
        pdf.cell(
            200, 10,
            txt=f"분석 일시: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
            ln=True
        )
        pdf.ln(10)
        ...
        for line in risks_clean.split('\n'):
            pdf.multi_cell(0, 10, line)

        pdf.ln(5)
        pdf.cell(200, 10, txt=f"종합 위험 점수: {score}%", ln=True)

        # 2) PDF 문자열 생성 → bytes로 변환
        pdf_str = pdf.output(dest='S')  # 이 시점에 이미 Latin-1 호환만 남아 있음
